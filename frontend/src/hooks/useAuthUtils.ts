...
const useAuthUtils = () => {
  ...
  const strToUint8Array = useCallback((str: string): Uint8Array => {
  ...
  const base64To8Uint8Array = useCallback((str: string): Uint8Array => {
  ...
  const encryptPrivateKey = useCallback(
    async (aesKey: CryptoKey, privateKey: string): Promise<EncryptedData> => {
  ...
  const savePrivateKeyToLocalStorage = useCallback(
  ...
  const loadPrivateKeyFromLocalStorage = useCallback(async (): Promise<
    string | null
  > => {
  ...
  const signChallenge = async (privateKeyPem: any, challenge: string) => {
    ...
    console.log(binaryKey, base64To8Uint8Array(privateKeyPem));
